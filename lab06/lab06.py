    ...
    def report(self):
        ...
        if self.changed():
            ...
            if self.before > self.after:
                ...
                change = "increased"
            msg = change + f" {self.before}->{self.after}"
...
def make_change(amount, coins):
    ...
    if amount < smallest:
        ...
        return [smallest]
    option1 = make_change(amount - smallest, rest)
    if option1 is None:
        newCoins = dict(coins)
        del newCoins[smallest]
        option2 = make_change(amount, newCoins)
        return option2
    else:
        return [smallest] + option1
